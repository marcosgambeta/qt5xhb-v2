...
PROCEDURE Main()
   ...
   oModel:setCB(Qt_DisplayRole, {|nRow,nCol|iif(nCol == 1, aEstados[nRow + 1, 2], NIL)}) // conteúdo da célula (coluna 2)
   oModel:setCB(Qt_DecorationRole, {|nRow,nCol|iif(nCol == 0, QPixmap():new(aEstados[nRow + 1, 1]), NIL)}) // conteúdo da célula (coluna 1)
   oModel:setCB(Qt_SizeHintRole, {|nRow,nCol|iif(nCol == 0, QSize():new(150, 107), NIL)}) // tamanho da célula (coluna 1)

   oModel:setHorizontalHeaderCB(Qt_DisplayRole, {|nCol|{"Bandeira", "Estado"}[nCol + 1]}) // títulos das colunas
   oModel:setVerticalHeaderCB(Qt_DisplayRole, {|nRow|alltrim(str(nRow))}) // títulos das linhas
