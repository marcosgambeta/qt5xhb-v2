...
static Signals4 *s_signals = nullptr;

/*
  constructor
*/
Signals4::Signals4(QObject *parent) : QObject(parent)
{
  mutex = new QMutex();
}
...
int Signals4_connect_signal ( QObject * object, int signal, PHB_ITEM codeblock )
{
  ...
  {
    s_signals = new Signals4(QCoreApplication::instance());
  }
...
*/
bool Signals4_connection ( QObject * object, int signal )
{
  // cria objeto da classe Signals, caso não tenha sido criado
  if( s_signals == nullptr )
  {
  ...
  //{
    ...
    bool connected = Signals4_is_signal_connected( object, signal );
...
/*
        ...
        {
*/
          Signals4_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          ret = true;
/*
        }
        else
...
bool Signals4_connection ( QObject * object, int signal, int & indexOfCodeBlock )
{
  // cria objeto da classe Signals, caso não tenha sido criado
  if( s_signals == nullptr )
  {
...
/*
        ...
        {
*/
          indexOfCodeBlock = Signals4_connect_signal( object, signal, codeblock ); // se conectado, adiciona
          ret = true;
/*
        }
        else
...
*/
bool Signals4_disconnection ( QObject * object, int signal )
{
  // cria objeto da classe Signals, caso não tenha sido criado
  if( s_signals == nullptr )
  {
...
/*
    ...
    {
*/
      Signals4_disconnect_signal( object, signal ); // se desconectado, remove
      ret = true;
/*
    }
*/
...
HB_FUNC( QTXHB_SIGNALS4_SIZE_ACTIVE )
{
  if( s_signals != nullptr )
  {
    // inicializa contador
...
bool Signals4_store_connection ( QObject * object, int signal, QMetaObject::Connection connection )
{
  // cria objeto da classe Signals, caso não tenha sido criado
  if( s_signals == nullptr )
  {
...
bool Signals4_store_connection ( int index, QMetaObject::Connection connection )
{
  ...
  s_signals->list5[index] = connection;

  //return stored;
  return true;
}

QMetaObject::Connection Signals4_get_connection ( QObject * object, int signal )
{
  // cria objeto da classe Signals, caso não tenha sido criado
  if( s_signals == nullptr )
  {
