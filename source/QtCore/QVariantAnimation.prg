...
#define GET_PTR_FROM_SELF(p) auto p = qobject_cast<QVariantAnimation *>(Qt5xHb::getQObjectPointerFromSelfItem())
...
HB_FUNC_STATIC(QVARIANTANIMATION_ONVALUECHANGED)
{
  ...
  if (sender != nullptr) {
    auto indexOfSignal = sender->metaObject()->indexOfSignal("valueChanged(QVariant)");
    auto indexOfCodeBlock = -1;

    if (ISNUMPAR(1) && ISBLOCKORSYMBOL(1)) {
      if (Qt5xHb::Signals_connection(sender, indexOfSignal, indexOfCodeBlock)) {
        auto connection = QObject::connect(
            sender, &QVariantAnimation::valueChanged, [sender, indexOfCodeBlock](const QVariant &arg1) {
