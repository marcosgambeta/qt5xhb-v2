...
#define GET_PTR_FROM_SELF(p) auto p = qobject_cast<QFile *>(Qt5xHb::getQObjectPointerFromSelfItem())
  ...
  if (ISNUMPAR(1) && HB_ISNUM(1)) {
    ...
    if (obj != nullptr) {
      RBOOL(obj->open(static_cast<FILE *>(hb_parptr(1)), PQFILE_OPENMODE(2),
                      HB_ISNIL(3) ? QFile::DontCloseHandle : PQFILE_FILEHANDLEFLAGS(3)));
    }
    ...
    if (obj != nullptr) {
      RBOOL(obj->open(PINT(1), PQFILE_OPENMODE(2), HB_ISNIL(3) ? QFile::DontCloseHandle : PQFILE_FILEHANDLEFLAGS(3)));
    }
  } else {
    THROW_ERROR_3012();
  }
...
HB_FUNC_STATIC(QFILE_ENCODENAME)
{
  ...
  if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
    RQBYTEARRAY(QFile::encodeName(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
  } else {
    THROW_ERROR_3012();
  }
...
#endif
  } else if (ISBETWEEN(1, 2) && ISQSTRING(1) && ISQSTRINGORNIL(2)) {
