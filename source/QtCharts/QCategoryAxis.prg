...
#define GET_PTR_FROM_SELF(p) auto p = qobject_cast<QCategoryAxis *>(Qt5xHb::getQObjectPointerFromSelfItem())
...
HB_FUNC_STATIC(QCATEGORYAXIS_SETLABELSPOSITION)
{
  ...
  if (obj != nullptr) {
    ...
    if (ISNUMPAR(1) && HB_ISNUM(1)) {
#endif
      obj->setLabelsPosition(PQCATEGORYAXIS_AXISLABELSPOSITION(1));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
  ...
  if (sender != nullptr) {
    ...
    if (ISNUMPAR(1) && ISBLOCKORSYMBOL(1)) {
      if (Qt5xHb::Signals_connection(sender, indexOfSignal, indexOfCodeBlock)) {
        auto connection = QObject::connect(sender, &QCategoryAxis::categoriesChanged, [sender, indexOfCodeBlock]() {
          auto cb = Qt5xHb::Signals_return_codeblock(indexOfCodeBlock);

          if (cb != nullptr) {
            auto pSender = Qt5xHb::Signals_return_qobject(sender, "QCATEGORYAXIS");
...
HB_FUNC_STATIC(QCATEGORYAXIS_ONLABELSPOSITIONCHANGED)
{
  ...
  if (sender != nullptr) {
    auto indexOfSignal =
        sender->metaObject()->indexOfSignal("labelsPositionChanged(QCategoryAxis::AxisLabelsPosition)");
    auto indexOfCodeBlock = -1;

    if (ISNUMPAR(1) && ISBLOCKORSYMBOL(1)) {
      if (Qt5xHb::Signals_connection(sender, indexOfSignal, indexOfCodeBlock)) {
        auto connection = QObject::connect(sender, &QCategoryAxis::labelsPositionChanged,
                                           [sender, indexOfCodeBlock](QCategoryAxis::AxisLabelsPosition arg1) {
                                             auto cb = Qt5xHb::Signals_return_codeblock(indexOfCodeBlock);
