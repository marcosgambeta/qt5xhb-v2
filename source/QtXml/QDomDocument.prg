//
// Qt5xHb - Bindings libraries for Harbour/xHarbour and Qt Framework 5
//
// Copyright (c) 2026 Marcos Antonio Gambeta <marcosgambeta@outlook.com>
//

// DO NOT EDIT THIS FILE - the content was created using a source code generator

// clang-format off

#include <hbclass.ch>

#ifndef QT5XHB_NO_REQUESTS
REQUEST QByteArray
REQUEST QDomAttr
REQUEST QDomCDATASection
REQUEST QDomComment
REQUEST QDomDocumentFragment
REQUEST QDomDocumentType
REQUEST QDomElement
REQUEST QDomEntityReference
REQUEST QDomImplementation
REQUEST QDomNode
REQUEST QDomNodeList
REQUEST QDomProcessingInstruction
REQUEST QDomText
#endif

CLASS QDomDocument INHERIT QDomNode

   METHOD new
   METHOD delete
   METHOD createAttribute
   METHOD createAttributeNS
   METHOD createCDATASection
   METHOD createComment
   METHOD createDocumentFragment
   METHOD createElement
   METHOD createElementNS
   METHOD createEntityReference
   METHOD createProcessingInstruction
   METHOD createTextNode
   METHOD doctype
   METHOD documentElement
   METHOD elementById
   METHOD elementsByTagName
   METHOD elementsByTagNameNS
   METHOD implementation
   METHOD importNode
   METHOD nodeType
   METHOD setContent
   METHOD toByteArray
   METHOD toString

ENDCLASS

// clang-format on

#pragma BEGINDUMP

#include <QtCore/Qt>

#ifndef __XHARBOUR__
#include <QtXml/QDomDocument>
#endif

#include "qt5xhb_common.hpp"
#include "qt5xhb_macros.hpp"
#include "qt5xhb_utils.hpp"

#ifdef __XHARBOUR__
#include <QtXml/QDomDocument>
#endif

#define GET_PTR_FROM_SELF(p) auto p = static_cast<QDomDocument *>(Qt5xHb::itemGetPtrStackSelfItem())

HB_FUNC_STATIC(QDOMDOCUMENT_NEW)
{
  if (ISNUMPAR(0)) {
    // QDomDocument()
    auto obj = new QDomDocument();
    Qt5xHb::returnNewObject(obj, true);
  } else if (ISNUMPAR(1) && ISQSTRING(1)) {
    // QDomDocument(const QString &name)
    auto obj = new QDomDocument(PQSTRING(1));
    Qt5xHb::returnNewObject(obj, true);
  } else if (ISNUMPAR(1) && ISQDOMDOCUMENTTYPE(1)) {
    // QDomDocument(const QDomDocumentType &doctype)
    auto obj = new QDomDocument(*PQDOMDOCUMENTTYPE(1));
    Qt5xHb::returnNewObject(obj, true);
  } else if (ISNUMPAR(1) && ISQDOMDOCUMENT(1)) {
    // QDomDocument(const QDomDocument &x)
    auto obj = new QDomDocument(*PQDOMDOCUMENT(1));
    Qt5xHb::returnNewObject(obj, true);
  } else {
    THROW_ERROR_3012();
  }
}

HB_FUNC_STATIC(QDOMDOCUMENT_DELETE)
{
  GET_PTR_FROM_SELF(obj);
  DELETE_OBJECT(obj);
  RETURN_SELF();
}

// QDomAttr createAttribute(const QString &name)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEATTRIBUTE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMATTR(obj->createAttribute(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomAttr createAttributeNS(const QString &nsURI, const QString &qName)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEATTRIBUTENS)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(2) && ISQSTRING(1) && ISQSTRING(2)) {
#endif
      RQDOMATTR(obj->createAttributeNS(PQSTRING(1), PQSTRING(2)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomCDATASection createCDATASection(const QString &value)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATECDATASECTION)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMCDATASECTION(obj->createCDATASection(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomComment createComment(const QString &value)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATECOMMENT)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMCOMMENT(obj->createComment(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomDocumentFragment createDocumentFragment()
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEDOCUMENTFRAGMENT)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(0)) {
#endif
      RQDOMDOCUMENTFRAGMENT(obj->createDocumentFragment());
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomElement createElement(const QString &tagName)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEELEMENT)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMELEMENT(obj->createElement(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomElement createElementNS(const QString &nsURI, const QString &qName)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEELEMENTNS)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(2) && ISQSTRING(1) && ISQSTRING(2)) {
#endif
      RQDOMELEMENT(obj->createElementNS(PQSTRING(1), PQSTRING(2)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomEntityReference createEntityReference(const QString &name)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEENTITYREFERENCE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMENTITYREFERENCE(obj->createEntityReference(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomProcessingInstruction createProcessingInstruction(const QString &target, const QString &data)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATEPROCESSINGINSTRUCTION)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(2) && ISQSTRING(1) && ISQSTRING(2)) {
#endif
      RQDOMPROCESSINGINSTRUCTION(obj->createProcessingInstruction(PQSTRING(1), PQSTRING(2)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomText createTextNode(const QString &value)
HB_FUNC_STATIC(QDOMDOCUMENT_CREATETEXTNODE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMTEXT(obj->createTextNode(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomDocumentType doctype() const
HB_FUNC_STATIC(QDOMDOCUMENT_DOCTYPE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(0)) {
#endif
      RQDOMDOCUMENTTYPE(obj->doctype());
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomElement documentElement() const
HB_FUNC_STATIC(QDOMDOCUMENT_DOCUMENTELEMENT)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(0)) {
#endif
      RQDOMELEMENT(obj->documentElement());
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomElement elementById(const QString &elementId)
HB_FUNC_STATIC(QDOMDOCUMENT_ELEMENTBYID)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMELEMENT(obj->elementById(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomNodeList elementsByTagName(const QString &tagname) const
HB_FUNC_STATIC(QDOMDOCUMENT_ELEMENTSBYTAGNAME)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(1) && ISQSTRING(1)) {
#endif
      RQDOMNODELIST(obj->elementsByTagName(PQSTRING(1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomNodeList elementsByTagNameNS(const QString &nsURI, const QString &localName)
HB_FUNC_STATIC(QDOMDOCUMENT_ELEMENTSBYTAGNAMENS)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(2) && ISQSTRING(1) && ISQSTRING(2)) {
#endif
      RQDOMNODELIST(obj->elementsByTagNameNS(PQSTRING(1), PQSTRING(2)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomImplementation implementation() const
HB_FUNC_STATIC(QDOMDOCUMENT_IMPLEMENTATION)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(0)) {
#endif
      RQDOMIMPLEMENTATION(obj->implementation());
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomNode importNode(const QDomNode &importedNode, bool deep)
HB_FUNC_STATIC(QDOMDOCUMENT_IMPORTNODE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(2) && ISQDOMNODE(1) && ISBOOL(2)) {
#endif
      RQDOMNODE(obj->importNode(*PQDOMNODE(1), PBOOL(2)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QDomNode::NodeType nodeType() const
HB_FUNC_STATIC(QDOMDOCUMENT_NODETYPE)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISNUMPAR(0)) {
#endif
      RENUM(obj->nodeType());
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

HB_FUNC_STATIC(QDOMDOCUMENT_SETCONTENT)
{
  if (ISBETWEEN(2, 5) && ISQBYTEARRAY(1) && ISBOOL(2) && ISQSTRINGORNIL(3) && ISNUMORNIL(4) && ISNUMORNIL(5)) {
    // bool setContent(const QByteArray &data, bool namespaceProcessing, QString *errorMsg = nullptr, int *errorLine =
    // nullptr, int *errorColumn = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par4;
      int par5;
      RBOOL(obj->setContent(*PQBYTEARRAY(1), PBOOL(2), nullptr, &par4, &par5));
      hb_storni(par4, 4);
      hb_storni(par5, 5);
    }
  } else if (ISBETWEEN(2, 5) && ISQSTRING(1) && ISBOOL(2) && ISQSTRINGORNIL(3) && ISNUMORNIL(4) && ISNUMORNIL(5)) {
    // bool setContent(const QString &text, bool namespaceProcessing, QString *errorMsg = nullptr, int *errorLine =
    // nullptr, int *errorColumn = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par4;
      int par5;
      RBOOL(obj->setContent(PQSTRING(1), PBOOL(2), nullptr, &par4, &par5));
      hb_storni(par4, 4);
      hb_storni(par5, 5);
    }
  } else if (ISBETWEEN(2, 5) && ISQIODEVICE(1) && ISBOOL(2) && ISQSTRINGORNIL(3) && ISNUMORNIL(4) && ISNUMORNIL(5)) {
    // bool setContent(QIODevice *dev, bool namespaceProcessing, QString *errorMsg = nullptr, int *errorLine = nullptr,
    // int *errorColumn = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par4;
      int par5;
      RBOOL(obj->setContent(PQIODEVICE(1), PBOOL(2), nullptr, &par4, &par5));
      hb_storni(par4, 4);
      hb_storni(par5, 5);
    }
  } else if (ISBETWEEN(2, 5) && ISQXMLINPUTSOURCE(1) && ISBOOL(2) && ISQSTRINGORNIL(3) && ISNUMORNIL(4) &&
             ISNUMORNIL(5)) {
    // bool setContent(QXmlInputSource *source, bool namespaceProcessing, QString *errorMsg = nullptr, int *errorLine =
    // nullptr, int *errorColumn = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par4;
      int par5;
      RBOOL(obj->setContent(PQXMLINPUTSOURCE(1), PBOOL(2), nullptr, &par4, &par5));
      hb_storni(par4, 4);
      hb_storni(par5, 5);
    }
  } else if (ISBETWEEN(1, 4) && ISQSTRING(1) && ISQSTRINGORNIL(2) && ISNUMORNIL(3) && ISNUMORNIL(4)) {
    // bool setContent(const QString &text, QString *errorMsg = nullptr, int *errorLine = nullptr, int *errorColumn =
    // nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par3;
      int par4;
      RBOOL(obj->setContent(PQSTRING(1), nullptr, &par3, &par4));
      hb_storni(par3, 3);
      hb_storni(par4, 4);
    }
  } else if (ISBETWEEN(1, 4) && ISQBYTEARRAY(1) && ISQSTRINGORNIL(2) && ISNUMORNIL(3) && ISNUMORNIL(4)) {
    // bool setContent(const QByteArray &buffer, QString *errorMsg = nullptr, int *errorLine = nullptr, int *errorColumn
    // = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par3;
      int par4;
      RBOOL(obj->setContent(*PQBYTEARRAY(1), nullptr, &par3, &par4));
      hb_storni(par3, 3);
      hb_storni(par4, 4);
    }
  } else if (ISBETWEEN(1, 4) && ISQIODEVICE(1) && ISQSTRINGORNIL(2) && ISNUMORNIL(3) && ISNUMORNIL(4)) {
    // bool setContent(QIODevice *dev, QString *errorMsg = nullptr, int *errorLine = nullptr, int *errorColumn =
    // nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par3;
      int par4;
      RBOOL(obj->setContent(PQIODEVICE(1), nullptr, &par3, &par4));
      hb_storni(par3, 3);
      hb_storni(par4, 4);
    }
  } else if (ISBETWEEN(2, 5) && ISQXMLINPUTSOURCE(1) && ISQXMLREADER(2) && ISQSTRINGORNIL(3) && ISNUMORNIL(4) &&
             ISNUMORNIL(5)) {
    // bool setContent(QXmlInputSource *source, QXmlReader *reader, QString *errorMsg = nullptr, int *errorLine =
    // nullptr, int *errorColumn = nullptr)
    GET_PTR_FROM_SELF(obj);

    if (obj != nullptr) {
      int par4;
      int par5;
      RBOOL(obj->setContent(PQXMLINPUTSOURCE(1), PQXMLREADER(2), nullptr, &par4, &par5));
      hb_storni(par4, 4);
      hb_storni(par5, 5);
    }
  } else {
    THROW_ERROR_3012();
  }
}

// QByteArray toByteArray(int indent = 1) const
HB_FUNC_STATIC(QDOMDOCUMENT_TOBYTEARRAY)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISBETWEEN(0, 1) && ISNUMORNIL(1)) {
#endif
      RQBYTEARRAY(obj->toByteArray(OPINT(1, 1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

// QString toString(int indent = 1) const
HB_FUNC_STATIC(QDOMDOCUMENT_TOSTRING)
{
  GET_PTR_FROM_SELF(obj);

  if (obj != nullptr) {
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    if (ISBETWEEN(0, 1) && ISNUMORNIL(1)) {
#endif
      RQSTRING(obj->toString(OPINT(1, 1)));
#ifndef QT5XHB_DONT_CHECK_PARAMETERS
    } else {
      THROW_ERROR_3012();
    }
#endif
  }
}

#pragma ENDDUMP
