...
#define GET_PTR_FROM_SELF(p) auto p = qobject_cast<QSensorGestureRecognizer *>(Qt5xHb::getQObjectPointerFromSelfItem())
    ...
    if (ISNUMPAR(1) && ISBLOCKORSYMBOL(1)) {
      if (Qt5xHb::Signals_connection(sender, indexOfSignal, indexOfCodeBlock)) {
        auto connection = QObject::connect(
            sender, &QSensorGestureRecognizer::detected, [sender, indexOfCodeBlock](const QString &arg1) {
              auto cb = Qt5xHb::Signals_return_codeblock(indexOfCodeBlock);

              if (cb != nullptr) {
                auto pSender = Qt5xHb::Signals_return_qobject(sender, "QSENSORGESTURERECOGNIZER");
